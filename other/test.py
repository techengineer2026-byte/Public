...
class YouTubeChannelDownloader:
    def __init__(self, root):
        ...
        self.videos = []           # video info dicts
        self.check_vars = []       # BooleanVar per video
        self.thumbnail_refs = []   # prevent garbage collection
    ...
    def _display_videos(self):
        ...
        self.status_lbl.config(text=f"✅ Loaded {len(self.videos)} videos")
        self._update_count()
    ...
    def _make_row(self, idx, vid):
        ...
        # thumbnail (placeholder → replaced)
        thumb_lbl = tk.Label(row, bg="#3d3d5c", width=10, height=5,
                             relief="flat")
        ...
        # fallback to known YouTube thumbnail URL
    ...
    def start_download(self):
        ...
        if not save_dir or not os.path.isdir(save_dir):
            messagebox.showwarning("Bad folder",
                                   "Choose a valid save folder.")
            return
